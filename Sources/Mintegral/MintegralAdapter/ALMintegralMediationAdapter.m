...
{
    // Guard against missing -ObjC linker flag which causes "unrecognized selector" crashes
    // (e.g., -[MTGDeviceInfo performBlockOnMainThread_by_mv:]) when Mintegral SDK categories are stripped.
    if ( !NSClassFromString(@"MTGSDK") )
    {
        NSString *message = @"Mintegral SDK not found. Ensure -ObjC is added to Other Linker Flags.";
        [self log: @"%@", message];
        completionHandler(MAAdapterInitializationStatusInitializedFailure, message);
        return;
    }
    ...
    dispatch_once(&onceToken, ^{
        ...
        {
            [self log: @"Mintegral SDK initialization failed with exception: %@. Ensure -ObjC is added to Other Linker Flags.", exception];
        }
    });
